...
void SANA::initTau(void) {
    ...
    0};
     */
    tau = vector<double>{0.996738, 0.994914, 0.993865, 0.974899, 0.977274, 0.980926, 0.97399,  0.970583, 0.967492, 0.962373, 0.953197,  0.954104,   0.951387,  0.953532,  0.948492, 0.939501, 0.939128,
                         0.932902, 0.912378, 0.896011, 0.89535,  0.88642,  0.874628, 0.856721, 0.855782, 0.838483, 0.820407, 0.784303,  0.771297,   0.751457,  0.735902,  0.676393, 0.633939, 0.604872,
                         0.53482,  0.456856, 0.446905, 0.377708, 0.337258, 3.04e-01, 0.280585, 0.240093, 1.95e-01, 1.57e-01, 1.21e-01,  1.00e-01,   8.04e-02,  5.95e-02,  4.45e-02, 3.21e-02, 1.81e-02,
                         1.82e-02, 1.12e-02, 7.95e-03, 4.82e-03, 3.73e-03, 2.11e-03, 1.41e-03, 9.69e-04, 6.96e-04, 5.48e-04, 4.20e-04,  4.00e-04,   3.50e-04,  3.10e-04,  2.84e-04, 2.64e-04, 1.19e-04,
                         8.16e-05, 7.22e-05, 6.16e-05, 4.46e-05, 3.36e-05, 2.66e-05, 1.01e-05, 9.11e-06, 4.09e-06, 3.96e-06, 3.43e-06,  3.12e-06,   2.46e-06,  2.02e-06,  1.85e-06, 1.72e-06, 1.10e-06,
                         9.13e-07, 8.65e-07, 8.21e-07, 7.26e-07, 6.25e-07, 5.99e-07, 5.42e-07, 8.12e-08, 4.16e-08, 6.56e-09, 9.124e-10, 6.1245e-10, 3.356e-10, 8.124e-11, 4.587e-11};
}

SANA::SANA(Graph *G1, Graph *G2, double TInitial, double TDecay, double t, bool usingIterations, bool addHillClimbing, MeasureCombination *MC, string &objectiveScore
#ifdef WEIGHTED
           ,
...
#error must have CORES macro defined to print them
        ...
        for (uint i = 0; i < n1; i++)
            for (uint j = 0; j < n2; j++)
                if (coreFreq[i][j] > 0) {
                    double unweightdedScore = coreFreq[i][j] / (double)coreCount[i], weightedScore = weightedCoreFreq[i][j] / totalCoreWeight[i];
                    if (weightedScore > MIN_CORE_SCORE)
                        printf("%s %s  %lu / %lu = %.16f weighted %.16f / %.16f = %.16f\n", G1Index2Name[i].c_str(), G2Index2Name[j].c_str(), coreFreq[i][j], coreCount[i], unweightdedScore,
                               weightedCoreFreq[i][j], totalCoreWeight[i], weightedScore);
                }
#endif
        return align;
    }
...
inline uint SANA::G1RandomUnlockedNode_Fast() { return unLockedNodesG1[G1RandomUnlockedNodeDist(gen)]; }
...
inline uint SANA::G2RandomUnlockedNode_Fast() { return G2RandomUnassignedNode(gen); }
...
void SANA::describeParameters(ostream &sout) {
    ...
    if (!usingIterations) {
        sout << "Execution time: ";
        if (restart) {
            sout << minutesNewAlignments + minutesPerCandidate * numCandidates + minutesFinalist;
        } else {
            sout << minutes;
        }
        sout << "m" << endl;
...
string SANA::fileNameSuffix(const Alignment &A) { return "_" + extractDecimals(eval(A), 3); }
...
double SANA::acceptingProbability(double energyInc, double Temperature) { return energyInc >= 0 ? 1 : exp(energyInc / Temperature); }

double SANA::trueAcceptingProbability() { return vectorMean(sampledProbability); }
...
Alignment SANA::simpleRun(const Alignment &startA, double maxExecutionSeconds, long long int maxExecutionIterations, long long int &iter) {
    initDataStructures(startA);
    setInterruptSignal();
    for (;; ++iter) {
        Temperature = temperatureFunction(iter, TInitial, TDecay);
...
unordered_set<vector<uint> *> *SANA::simpleParetoRun(const Alignment &startA, double maxExecutionSeconds, long long int &iter, const string &fileName) {
...
unordered_set<vector<uint> *> *SANA::simpleParetoRun(const Alignment &startA, long long int maxExecutionIterations, long long int &iter, const string &fileName) {
...
unordered_map<string, int> SANA::mapScoresToIndexes() {
    ...
    // measureNames.resize(distance(measureNames.begin(), unique(measureNames.begin(), measureNames.end()))); //Removes any duplicate measure names. Somehow, ICS measure appears twice... a good check
    // to perform.
...
#ifdef WEIGHTED
    for (uint i = 0; i < numOfMeasures; i++) {
        addScores[scoreNamesToIndexes[measureNames[i]]] = measureCalculation[measureNames[i]](aligEdges, g1Edges, inducedEdges, g2Edges, TCSum, localScoreSum, n1, wecSum, ewecSum, ncSum, trueA.back(),
                                                                                              g1WeightedEdges, g2WeightedEdges, squaredAligEdges);
    }
#else
    for (uint i = 0; i < numOfMeasures; i++) {
        addScores[scoreNamesToIndexes[measureNames[i]]] =
            measureCalculation[measureNames[i]](aligEdges, g1Edges, inducedEdges, g2Edges, TCSum, localScoreSum, n1, wecSum, ewecSum, ncSum, trueA.back());
    }
#endif
...
vector<double> SANA::getMeasureScores(double newAligEdges, double newInducedEdges, double newTCSum, double newLocalScoreSum, double newWecSum, double newNcSum, double newEwecSum,
                                      double newSquaredAligEdges) {
    vector<double> addScores(numOfMeasures);
#ifdef WEIGHTED
    for (uint i = 0; i < numOfMeasures; i++) {
        addScores[scoreNamesToIndexes[measureNames[i]]] = measureCalculation[measureNames[i]](newAligEdges, g1Edges, newInducedEdges, g2Edges, newTCSum, newLocalScoreSum, n1, newWecSum, newEwecSum,
                                                                                              newNcSum, trueA.back(), g1WeightedEdges, g2WeightedEdges, newSquaredAligEdges);
    }
#else
    for (uint i = 0; i < numOfMeasures; i++) {
        addScores[scoreNamesToIndexes[measureNames[i]]] =
            measureCalculation[measureNames[i]](newAligEdges, g1Edges, newInducedEdges, g2Edges, newTCSum, newLocalScoreSum, n1, newWecSum, newEwecSum, newNcSum, trueA.back());
    }
#endif
...
void SANA::insertCurrentAlignmentAndData() {
    ...
    if (needLocal)
        storedLocalScoreSumMap[A] = localScoreSumMap;
    if (needWec)
...
void SANA::removeAlignmentData(vector<uint> *toRemove) {
    ...
    if (needAligEdges or needSec)
        storedAligEdges.erase(toRemove);
    if (needSquaredAligEdges)
        storedSquaredAligEdges.erase(toRemove);
    if (needInducedEdges)
        storedInducedEdges.erase(toRemove);
    if (needTC)
        storedTCSum.erase(toRemove);
    if (needLocal)
        storedLocalScoreSum.erase(toRemove);
    if (needLocal)
        storedLocalScoreSumMap.erase(toRemove);
    if (needWec)
        storedWecSum.erase(toRemove);
    if (needEwec)
        storedEwecSum.erase(toRemove);
    if (needNC)
        storedNcSum.erase(toRemove);
...
bool SANA::scoreComparison(double newAligEdges, double newInducedEdges, double newTCSum, double newLocalScoreSum, double newWecSum, double newNcSum, double &newCurrentScore, double newEwecSum,
                           double newSquaredAligEdges) {
    ...
    case Score::max: {
        ...
        double deltaEnergy = max(ncWeight * (newNcSum / trueA.back() - ncSum / trueA.back()),
                                 max(max(ecWeight * (newAligEdges / g1Edges - aligEdges / g1Edges),
                                         max(s3Weight * ((newAligEdges / (g1Edges + newInducedEdges - newAligEdges) - (aligEdges / (g1Edges + inducedEdges - aligEdges)))),
                                             secWeight * 0.5 * (newAligEdges / g1Edges - aligEdges / g1Edges + newAligEdges / g2Edges - aligEdges / g2Edges))),
                                     max(localWeight * ((newLocalScoreSum / n1) - (localScoreSum)), wecWeight * (newWecSum / (2 * g1Edges) - wecSum / (2 * g1Edges)))));
    ...
    case Score::min: {
        ...
        assert(icsWeight == 0.0);
        double deltaEnergy = min(ncWeight * (newNcSum / trueA.back() - ncSum / trueA.back()),
                                 min(min(ecWeight * (newAligEdges / g1Edges - aligEdges / g1Edges),
                                         min(s3Weight * ((newAligEdges / (g1Edges + newInducedEdges - newAligEdges) - (aligEdges / (g1Edges + inducedEdges - aligEdges)))),
                                             secWeight * 0.5 * (newAligEdges / g1Edges - aligEdges / g1Edges + newAligEdges / g2Edges - aligEdges / g2Edges))),
                                     min(localWeight * ((newLocalScoreSum / n1) - (localScoreSum)), wecWeight * (newWecSum / (2 * g1Edges) - wecSum / (2 * g1Edges)))));
    ...
    case Score::maxFactor: {
        assert(icsWeight == 0.0);
        double maxScore = max(ncWeight * (newNcSum / trueA.back() - ncSum / trueA.back()),
                              max(max(ecWeight * (newAligEdges / g1Edges - aligEdges / g1Edges),
                                      max(s3Weight * ((newAligEdges / (g1Edges + newInducedEdges - newAligEdges) - (aligEdges / (g1Edges + inducedEdges - aligEdges)))),
                                          secWeight * 0.5 * (newAligEdges / g1Edges - aligEdges / g1Edges + newAligEdges / g2Edges - aligEdges / g2Edges))),
                                  max(localWeight * ((newLocalScoreSum / n1) - (localScoreSum)), wecWeight * (newWecSum / (2 * g1Edges) - wecSum / (2 * g1Edges)))));

        double minScore = min(ncWeight * (newNcSum / trueA.back() - ncSum / trueA.back()),
                              min(min(ecWeight * (newAligEdges / g1Edges - aligEdges / g1Edges),
                                      min(s3Weight * ((newAligEdges / (g1Edges + newInducedEdges - newAligEdges) - (aligEdges / (g1Edges + inducedEdges - aligEdges)))),
                                          secWeight * 0.5 * (newAligEdges / g1Edges - aligEdges / g1Edges + newAligEdges / g2Edges - aligEdges / g2Edges))),
                                  min(localWeight * ((newLocalScoreSum / n1) - (localScoreSum)), wecWeight * (newWecSum / (2 * g1Edges) - wecSum / (2 * g1Edges)))));
    ...
    case Score::pareto: { // This determines whether we should update the current alignment.
    ...
    if (wasBadMove &&
        (iterationsPerformed % 512 == 0 || (TCWeight > 0 && iterationsPerformed % 32 == 0))) { // this will never run in the case of iterationsPerformed never being changed so that it doesn't greatly
...
double SANA::localScoreSumIncChangeOp(vector<vector<float>> const &sim, uint const &source, uint const &oldTarget, uint const &newTarget) { return sim[source][newTarget] - sim[source][oldTarget]; }
...
int SANA::ncIncChangeOp(uint source, uint oldTarget, uint newTarget) {
    int change = 0;
    if (trueA[source] == oldTarget)
        change -= 1;
    if (trueA[source] == newTarget)
        change += 1;
    return change;
}
...
int SANA::ncIncSwapOp(uint source1, uint source2, uint target1, uint target2) {
    int change = 0;
    if (trueA[source1] == target1)
        change -= 1;
    if (trueA[source2] == target2)
        change -= 1;
    if (trueA[source1] == target2)
        change += 1;
    if (trueA[source2] == target1)
        change += 1;
    return change;
}

void SANA::trackProgress(long long int i, bool end) {
    if (!enableTrackProgress)
        return;
    ...
    if (not(printDetails or printScores or checkScores))
        return;
    Alignment Al(*A);
    ...
    if (dynamic_tdecay) { // Code for estimating dynamic TDecay
        ...
        if (abs(1 - ratio) >= .01 && (ratio < 1 || SANAtime > .2)) // don't speed it up too soon
        {
            double shouldBe;
...
#ifdef CORES
vector<ulong> SANA::getCoreCount() { return coreCount; }
Matrix<ulong> SANA::getCoreFreq() { return coreFreq; }
#endif
...
void SANA::searchTemperaturesByLinearRegression() {
    ...
    cout << "Increasing sample density near tFinal. "
         << "left bound: " << pow(10, binarySearchLeftEnd) << ", right bound: " << pow(10, binarySearchRightEnd) << endl;
    for (int j = 0; j < 4; ++j) {
    ...
    mid                  = (binarySearchRightEnd + binarySearchLeftEnd) / 2;
    cout << "Increasing sample density near tInitial. "
         << "left bound: " << pow(10, binarySearchLeftEnd) << ", right bound: " << pow(10, binarySearchRightEnd) << endl;
    for (int j = 0; j < 4; ++j) {
        double temperature = pow(10, mid);
        double probability = pForTInitial(temperature);
...
void SANA::searchTemperaturesByStatisticalTest() {
    ...
    uint count = 0;
    TImer.start();
    while (fabs(lowerBoundTInitial - upperBoundTInitial) / lowerBoundTInitial > 0.05 and count <= 100) {
        // search in log space
...
void SANA::setAcceptableTInitial() { TInitial = findAcceptableTInitial(TInitial); }

void SANA::setAcceptableTFinal() { TFinal = findAcceptableTFinal(TFinal); }

void SANA::setAcceptableTFinalFromManualTInitial() { TFinal = findAcceptableTFinalFromManualTInitial(TInitial); }
...
void SANA::setTDecay(double t) { TDecay = t; }

void SANA::setTDecayAutomatically() { TDecay = solveTDecay(); }
...
double SANA::getPforTInitial(const Alignment &startA, double maxExecutionSeconds, long long int &iter) {
    double result = 0.0;
    initDataStructures(startA);
    ...
    for (;; ++iter) {
        ...
        } // This is somewhat redundant with iter, but this is specifically for counting total iterations in the entire SANA object.  If you want this changed, post a comment on one of Dillon's
          // commits and he'll make it less redundant but he needs here for now.
        SANAIteration();
    }
...
bool SANA::isRandomTInitial(double TInitial, double highThresholdScore, double lowThresholdScore) {
    ...
    if (score > highThresholdScore)
        return false;
    if (score < lowThresholdScore)
        return true;
    ...
    for (uint i = 0; i < NUM_SAMPLES; ++i) {
        if (scoreForTInitial(TInitial) <= lowThresholdScore)
            return true;
    }
    return false;
}

double SANA::scoreRandom() { return eval(Alignment::randomAlignmentWithLocking(G1, G2)); }
...
Alignment SANA::hillClimbingAlignment(long long int idleCountTarget) { return hillClimbingAlignment(getStartingAlignment(), idleCountTarget); }
...
void SANA::initIterPerSecond() {
    ...
    cout << "SANA does " << to_string(res) << " iterations per second (took " << TImer.elapsedString() << " doing " << iter << " iterations)" << endl;
...
void SANA::setDynamicTDecay() { dynamic_tdecay = true; }
...
void SANA::prune(string &startAligName) {
    ...
    for (auto c : G2EdgeList) {
        if (removedEdges.find(pair<uint, uint>(c[0], c[1])) != removedEdges.end() or removedEdges.find(pair<uint, uint>(c[1], c[0])) != removedEdges.end()) {
            continue;
        }
...
unordered_set<vector<uint> *> *SANA::parallelParetoRun(const Alignment &startA, long long int maxExecutionIterations, const string &fileName) {
    initDataStructures(startA);
    setInterruptSignal();
    vector<double> scores;
    scoreNamesToIndexes = mapScoresToIndexes();
...
unordered_set<vector<uint> *> *SANA::parallelParetoRun(const Alignment &startA, double maxExecutionSeconds, const string &fileName) {
    initDataStructures(startA);
    setInterruptSignal();
    vector<double> scores;
    scoreNamesToIndexes = mapScoresToIndexes();
...
double SANA::trueAcceptingProbability(Job &job) { return vectorMean(job.sampledProbability); }
...
void SANA::storeAlignment(Job &job) {
    ...
    if (needAligEdges or needSec)
        storedAligEdges[A] = info.aligEdges;
    if (needSquaredAligEdges)
        storedSquaredAligEdges[A] = info.squaredAligEdges;
    if (needInducedEdges)
        storedInducedEdges[A] = info.inducedEdges;
    if (needTC)
        storedTCSum[A] = info.TCSum;
    if (needLocal)
        storedLocalScoreSum[A] = info.localScoreSum;
    if (needLocal)
        storedLocalScoreSumMap[A] = info.localScoreSumMap;
    if (needWec)
        storedWecSum[A] = info.wecSum;
    if (needEwec)
        storedEwecSum[A] = info.ewecSum;
    if (needNC)
        storedNcSum[A] = info.ncSum;
...
#ifdef WEIGHTED
    for (uint i = 0; i < numOfMeasures; i++) {
        addScores[scoreNamesToIndexes[measureNames[i]]] = measureCalculation[measureNames[i]](info.aligEdges, g1Edges, info.inducedEdges, g2Edges, info.TCSum, info.localScoreSum, n1, info.wecSum,
                                                                                              info.ewecSum, info.ncSum, trueA.back(), g1WeightedEdges, g2WeightedEdges, info.squaredAligEdges);
    }
#else
    for (uint i = 0; i < numOfMeasures; i++) {
        addScores[scoreNamesToIndexes[measureNames[i]]] =
            measureCalculation[measureNames[i]](info.aligEdges, g1Edges, info.inducedEdges, g2Edges, info.TCSum, info.localScoreSum, n1, info.wecSum, info.ewecSum, info.ncSum, trueA.back());
    }
#endif
    return addScores;
}
...
double SANA::getElapsedTime() {
    ...
    double seconds                           = chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count() * 1.0 / 1000;
    return seconds;
}
...
mutex quick_lock;
bool SANA::scoreComparison(Job &job, double newAligEdges, double newInducedEdges, double newTCSum, double newLocalScoreSum, double newWecSum, double newNcSum, double &newCurrentScore,
    ...
    if (wasBadMove && (job.iterationsPerformed % 512 == 0 ||
                       (TCWeight > 0 && job.iterationsPerformed % 32 == 0))) { // this will never run in the case of iterationsPerformed never being changed so that it doesn't greatly slow down the
                                                                               // program if for some reason iterationsPerformed doesn't need to be changed.
...
inline uint SANA::G1RandomUnlockedNode_Fast(Job &job) { return unLockedNodesG1[G1RandomUnlockedNodeDist(job.gen)]; }
...
inline uint SANA::G2RandomUnlockedNode_Fast(Job &job) { return G2RandomUnassignedNode(job.gen); }
