...
void Graph::serializeGraph(Graph &G, string outputName, bool typedNodes, bool locked) {
    ...
    if (fd != -1) {
        ...
        string outName = "networks/" + outputName + "/autogenerated/" + outputName + (typedNodes ? "_TypedNodes" : "_UntypedNodes") + (locked ? "_Locked" : "_Unlocked") +
                         (weighted ? "_Weighted" : "_Unweighted") + ".bin";

        string tmpName =
            "/tmp/" + outputName + (typedNodes ? "_TypedNodes" : "_UntypedNodes") + (locked ? "_Locked" : "_Unlocked") + (weighted ? "_Weighted" : "_Unweighted") + ".bin" + to_string(getpid());
...
void Graph::loadGraphFromBinary(Graph &g, string graphName, string lockFile, bool nodesHaveTypes, bool lockedSameName) {
    ...
    ifstream ifs("networks/" + graphName + "/autogenerated/" + graphName + (nodesHaveTypes ? "_TypedNodes" : "_UntypedNodes") + (locked ? "_Locked" : "_Unlocked") +
                     (weighted ? "_Weighted" : "_Unweighted") + ".bin",
...
Graph::Graph()
    : nodeTypes(vector<int>(0)), geneCount(0), miRNACount(0), unlockedGeneCount(0), unlockedmiRNACount(0), geneIndexList(vector<uint>(0)), miRNAIndexList(vector<uint>(0)),
      edgeList(vector<vector<uint>>(0)), matrix(0), adjLists(vector<vector<uint>>(0)), connectedComponents(vector<vector<uint>>(0)), lockedList(vector<bool>(0)), lockedTo(vector<string>(0)),
      lockedCount(0), nodeNameToIndexMap(unordered_map<string, uint>(0)) {}
...
void Graph::getMatrix(Matrix<MATRIX_UNIT> &matrixCopy) const { matrixCopy = matrix; }

void Graph::getAdjLists(vector<vector<uint>> &adjListsCopy) const { adjListsCopy = vector<vector<uint>>(adjLists); }

void Graph::getEdgeList(vector<vector<uint>> &edgeListCopy) const { edgeListCopy = edgeList; }

const vector<vector<uint>> &Graph::getConnectedComponents() const { return connectedComponents; }

void Graph::setMatrix(Matrix<MATRIX_UNIT> &matrixCopy) { matrix = matrixCopy; }

void Graph::setAdjLists(vector<vector<uint>> &adjListsCopy) { adjLists = adjListsCopy; }
void Graph::setEdgeList(vector<vector<uint>> &edgeListCopy) { edgeList = edgeListCopy; }
...
bool comp_vectors(const vector<uint> &a, const vector<uint> &b) { return a.size() > b.size(); }
...
void Graph::initConnectedComponents() {
    ...
    while (nodes.size() > 0) {
        ...
        if (nodesAreChecked[startOfConnected])
            continue;
...
uint Graph::randomNode() { return randInt(0, getNumNodes() - 1); }
...
void Graph::removeEdge(uint node1, uint node2) {
    ...
    // update edge list
    for (uint i = 0; i < m; i++) {
        if ((edgeList[i][0] == node1 and edgeList[i][1] == node2) or (edgeList[i][0] == node2 and edgeList[i][1] == node1)) {
...
string Graph::autogenFilesFolder() { return "networks/" + name + "/autogenerated/"; }
...
vector<uint> Graph::degreeDistribution() const {
    ...
    for (uint i = 1; i < n; i++) {
        if (adjLists[i].size() > maxDegree)
            maxDegree = adjLists[i].size();
    }
...
struct Point {
    ...
    double distOrigin() { return sqrt(x * x + y * y + z * z); }
...
void Graph::saveInGWFormat(string outputFile, const vector<string> &nodeNames, const vector<vector<uint>> &edgeList) {
    ofstream outfile;
...
void Graph::saveInGWFormatShuffled(string outputFile, const vector<string> &nodeNames, const vector<vector<uint>> &edgeList) {
...
void Graph::saveInGWFormatWithNames(string outputFile) { saveInGWFormat(outputFile, getNodeNames(), edgeList); }
...
bool Graph::isWellDefined() {
    ...
    for (uint i = 0; i < edgeList.size(); i++) {
        ...
        if (edgeList[i][0] < 0 or edgeList[i][0] >= numNodes or edgeList[i][1] < 0 or edgeList[i][1] >= numNodes) {
        ...
        for (uint j = 0; j < i; j++) {
            if (matrix.get(i, j)) {
                ...
                for (uint k = 0; not found and k < edgeList.size(); k++) {
                    if ((edgeList[k][0] == i and edgeList[k][1] == j) or (edgeList[k][1] == i and edgeList[k][0] == j))
                        found = true;
                }
                if (not found) {
                    cerr << "Node " << i << " adjacent to node " << j << " in adjacency";
...
vector<bool> &Graph::getLockedList() { return lockedList; }
bool Graph::isLocked(uint index) const { return lockedList[index]; }

string Graph::getLockedTo(uint index) { return lockedTo[index]; }

int Graph::getLockedCount() { return lockedCount; }
...
unordered_map<uint, uint> Graph::getNodeTypes_ReIndexMap() const {
    ...
    assert(lockedIndex == n - lockedCount - 1 and unlocked_gene_index == unlocked_gene_count and unlocked_miRNA_index == n - lockedCount);
    return result;
}
...
int Graph::getNodeType(uint i) { return nodeTypes[i]; }
